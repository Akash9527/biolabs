  ...
  private getInvoiceData(month: number, year: number) {
    ...
    this.invoiceService.getInvoiceByMonth(this.companyId, month, year).subscribe((response) => {
      ...
      for (const orderProduct of response.orderProducts) {
        orderProduct.order_product_startDate = (orderProduct.order_product_startDate) ?
          this.defaultDataService.dateWithoutTime(orderProduct.order_product_startDate) : null;
        orderProduct.order_product_endDate = (orderProduct.order_product_endDate) ?
          this.defaultDataService.dateWithoutTime(orderProduct.order_product_endDate) : null;

        orderProduct.editable = false;
        if ((orderProduct.order_product_endDate && orderProduct.order_product_currentCharge &&
          month == new Date(orderProduct.order_product_endDate).getMonth() + 1) ||
          (!orderProduct.order_product_endDate && orderProduct.order_product_currentCharge)
        ) {
          const start = (orderProduct.order_product_startDate &&
            (new Date(orderProduct.order_product_startDate).getMonth() + 1) == month) ?
            new Date(orderProduct.order_product_startDate) : new Date(year, month - 1, 1);

          const end = (orderProduct.order_product_endDate &&
            (new Date(orderProduct.order_product_endDate).getMonth() + 1) == month) ?
            new Date(orderProduct.order_product_endDate) :
            new Date(year, month - 1, monthDays);
